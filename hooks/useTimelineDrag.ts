...
export const useTimelineDrag = (
    ...
    const [dragState, setDragState] = useState<{
        ...
        prop?: string;
        id: string;
        type: 'trim' | 'move' | 'fade-in' | 'fade-out' | 'keyframe';
        edge?: 'start' | 'end';
    ...
    const handleMouseMove = useCallback((e: MouseEvent, timelineRef: React.RefObject<HTMLDivElement>) => {
        if (selectionBox) {
        ...
        const getAllSnapPoints = () => {
            ...
            if (state.snapSettings?.clips) {
                [...state.videoClips, ...state.audioTrack, ...state.textTrack, ...state.subtitleTrack].forEach(c => {
        ...
        if (dragState.type === 'trim' && dragState.trackName && dragState.initialDuration !== undefined) {
            ...
            if (dragState.edge === 'start') {
                ...
                newDuration = Math.max(0.1, dragState.initialDuration - (newStartTime - dragState.initialStartTime));
            } else {
            ...
            onTrim(dragState.trackName, dragState.id, newStartTime, newDuration);
            ...
            const newFade = Math.max(0, Math.min(dragState.initialDuration / 2, (dragState.initialFadeIn || 0) + dx));
            onUpdateClipFade(dragState.id, { fadeInDuration: newFade });
        } else if (dragState.type === 'fade-out') {
            const newFade = Math.max(0, Math.min(dragState.initialDuration / 2, (dragState.initialFadeOut || 0) - dx));
    ...
    return { dragState, setDragState, selectionBox, setSelectionBox, snapGuide, setSnapGuide, handleMouseMove, handleMouseUp };
};
