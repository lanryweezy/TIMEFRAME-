...
export const EditorView: React.FC = () => {
  ...
  const selectedAsset = state.assetManager?.library?.find(
    (a: any) => a.id === state.selectedAssetId,
  );
    ...
    <>
      <div className="flex flex-col h-full w-full bg-app-bg text-white overflow-hidden font-sans">
        ...
        <div className="flex-1 flex overflow-hidden">
              ...
              >
                <div className="w-[280px] h-full">
                  <EditorSidebar
                    ...
                    onSetFilter={(f: any) =>
                      setState((prev: any) => ({ ...prev, activeFilter: f }))
                    }
          ...
          {/* Main Workspace Area */}
          <main
            className="flex-1 flex flex-col relative min-w-0 bg-black"
          ...
          >
            <div className="flex-1 flex flex-col md:flex-row gap-2 p-2 relative overflow-hidden">
              ...
              {ui.activeWorkspace === 'editing' && (
                <button
                  onClick={(e) => {
                    e.stopPropagation();
                    setShowSourceMonitor(!showSourceMonitor);
                  }}
            ...
            <AnimatePresence>
              ...
              {(ui.activeWorkspace as string) === 'export' && <ExportLab />}
            </AnimatePresence>
                ...
                <div className="w-[320px] h-full flex flex-col">
                  <div className="flex bg-white/5 mx-4 mt-4 rounded-xl overflow-hidden p-1 gap-1">
                    ...
                    >
                      ...
                      <span className="text-[9px] font-bold uppercase tracking-wider text-inherit">
                        Properties
                      </span>
                    </button>
                    <button
                      ...
                      >
                        {rightPanel === 'properties' ? (
                          <PropertiesPanel
                            ...
                            onChangeChromaKey={(c: any) =>
                              setState((prev: any) => ({ ...prev, chromaKey: c }))
                            }
      ...
      <AnimatePresence>
        {state.isDetectingScenes && (
          <SceneDetector
            onClose={() => setState((prev: any) => ({ ...prev, isDetectingScenes: false }))}
          />
        )}
      </AnimatePresence>
        ...
        <div className="fixed top-4 left-1/2 transform -translate-x-1/2 z-[100] bg-zinc-900 border border-zinc-800 text-white px-6 py-4 rounded-xl shadow-2xl flex items-center gap-4">
          <div>
            ...
            <p className="text-xs text-zinc-400">
              Restore session from {new Date(state.pendingRecovery.timestamp).toLocaleTimeString()}?
            </p>
          </div>
