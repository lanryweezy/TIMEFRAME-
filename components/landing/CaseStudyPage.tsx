...
const caseStudies = {
    'case-study-1': {
        ...
        metrics: [{label: 'Reduction', val: '80%'}, {label: 'Time Saved', val: '34h'}, {label: 'Efficiency', val: '6.6x'}],
    ...
    'case-study-2': {
        ...
        metrics: [{label: 'Scale', val: '5x'}, {label: 'Monthly Output', val: '50+'}, {label: 'Cost/Video', val: '-60%'}],
    ...
    return (
        <div className="py-32 max-w-5xl mx-auto px-6 text-white">
            <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="mb-16 text-center">
                <span className="text-electric-blue font-bold uppercase tracking-widest text-sm">Case Study</span>
                <h1 className="text-6xl font-extrabold tracking-tighter mt-4">{study.title}</h1>
            </motion.div>
            
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.2 }} className="grid grid-cols-3 gap-6 mb-12">
                {study.metrics.map(m => (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-12">
                ...
                <div className="glass p-10 rounded-3xl">
                    ...
                    <ul className="space-y-3">
                        {study.steps.map(s => <li key={s} className="flex gap-2 items-center text-white"> <div className="w-1.5 h-1.5 rounded-full bg-electric-blue"/> {s}</li>)}
                    </ul>
                </div>
            </div>
        </div>
