...
const AutomationEditor = ({
  ...
  const handleCanvasClick = (e: React.MouseEvent<HTMLCanvasElement>) => {
    ...
    const newKeyframes = [
      ...keyframes,
      { id: Math.random().toString(36), time: newTime, value: newValue },
...
const AudioPanel: React.FC<AudioPanelProps> = ({ state, onUpdateAudio, handleSendMessage }) => {
  ...
  const AudioTrackControl = ({ audio, color }: { audio: AudioBlock; color: string }) => {
        ...
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="flex gap-1.5">
            ...
            <button
              ...
              title="Mute Audio"
              className="w-7 h-7 rounded-lg bg-zinc-800 border border-white/5 text-[9px] font-black hover:bg-studio-accent hover:text-black transition-all"
            >
              M
            </button>
          </div>
            ...
            >
              <div className="p-3 bg-black/40 rounded-xl border border-white/5 flex justify-between gap-2 h-32 items-end pb-6 relative overflow-hidden">
                ...
                <div className="absolute top-2 left-3 flex items-center gap-2 z-10">
                  ...
                  <span className="text-[8px] font-black uppercase text-zinc-500 tracking-widest">
                    5-Band Parametric EQ + Spectral Analysis
                  </span>
                </div>
                {['80Hz', '250Hz', '1kHz', '4kHz', '12kHz'].map((label, i) => (
                  <div
                    key={label}
                    className="flex-1 flex flex-col items-center gap-1.5 h-full justify-end group/eq z-10"
                  >
      ...
      <div className="p-5 bg-zinc-950/80 border-b border-studio-border backdrop-blur-md">
        <div className="flex justify-between items-center mb-6">
          ...
          <div className="flex items-center gap-2">
            <div className="flex flex-col items-end">
              ...
              <span
                className={`text-[10px] font-mono ${audioEngine.getMasterLUFS() > -14 ? 'text-red-500' : 'text-studio-accent'}`}
              >
        ...
        <div className="space-y-2">
          {[1, 2].map((ch) => (
            <div
              key={ch}
              className="h-2 rounded-full bg-black/60 p-[1px] border border-white/5 overflow-hidden"
            >
              <motion.div
                className="h-full bg-gradient-to-r from-studio-accent via-studio-accent to-red-500 rounded-full"
                animate={{
                  width: state.isPlaying
                    ? `${Math.min(100, masterLevel * (150 + ch * 10))}%`
                    : '2%',
                }}
        ...
        <AnimatePresence mode="wait">
              ...
              {audioTracks.length === 0 ? (
                <div className="flex flex-col items-center justify-center py-20 text-center space-y-4 border-2 border-dashed border-white/5 rounded-3xl">
                  ...
                  <div className="space-y-1">
                    <h3 className="text-[11px] font-black uppercase text-zinc-500">
                      No Audio Tracks
                    </h3>
                    <p className="text-[8px] text-zinc-600 uppercase font-bold tracking-widest">
                      Drag sounds into the timeline to begin mixing
                    </p>
                  </div>
                </div>
              ) : (
                <div className="space-y-4">
                  {audioTracks.map((audio) => (
                    <AudioTrackControl key={audio.id} audio={audio} color="bg-studio-accent" />
                  ))}
                </div>
              )}
                  ...
                  <div className="flex items-center justify-between mb-8">
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[7px] text-zinc-500 font-mono uppercase">
                          Stereo Output (L/R)
                        </span>
                      </div>
                    </div>
            ...
            >
              <div className="p-6 bg-gradient-to-r from-studio-accent/20 to-purple-500/20 border border-studio-accent/30 rounded-3xl group hover:border-studio-accent transition-all shadow-lg">
                ...
                <p className="text-[10px] text-zinc-400 leading-relaxed mb-6 uppercase font-bold tracking-tight">
                  Studio-grade AI processing for noise removal, stem separation, and rhythmic
                  intelligence.
                </p>
                <div className="grid grid-cols-1 gap-3">
                  <button
                    onClick={() =>
                      handleSendMessage('Run AI Mastering and normalize project to -14 LUFS.')
                    }
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <button
                      onClick={() =>
                        handleSendMessage('Apply AI Noise & Reverb removal to dialogue.')
                      }
                    ...
                    <button
                      onClick={() => {
                        handleSendMessage(
                          state.audioSettings.deClickEnabled
                            ? 'Disable project-wide De-Clicker.'
                            : 'Enable project-wide De-Clicker.',
                        );
                      }}
                    ...
                    <button
                      onClick={() =>
                        handleSendMessage('Split the music track into Vocals, Drums, and Bass.')
                      }
                    ...
                    <button
                      onClick={() =>
                        handleSendMessage('Analyze project loudness for Netflix/TikTok compliance.')
                      }
                    ...
                    <button
                      onClick={() =>
                        handleSendMessage('Generate a voiceover using my cloned voice.')
                      }
                  ...
                  <button
                    onClick={() =>
                      handleSendMessage('Detect BPM and add rhythm markers to the timeline.')
                    }
                ...
                <div className="mt-8 pt-8 border-t border-white/5 space-y-4">
                  <div className="flex items-center justify-between">
                    <span className="text-[10px] font-black uppercase tracking-widest text-studio-accent">
                  ...
                  <div className="relative group">
                    <input
                      ...
                      onKeyDown={(e) => {
                        if (e.key === 'Enter') {
                          handleSendMessage(
                            `Generate a custom sound effect: ${e.currentTarget.value}`,
                          );
                    ...
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 flex items-center gap-2 pointer-events-none">
                      <span className="text-[8px] font-black text-zinc-600 uppercase tracking-tighter bg-zinc-900 px-1.5 py-0.5 rounded border border-white/5">
                        Enter
                      </span>
                    </div>
                  </div>
              ...
              <div className="pt-4 border-t border-white/5 mt-4 space-y-6">
                <div className="p-5 bg-gradient-to-br from-studio-accent/20 to-indigo-900/20 border border-studio-accent/30 rounded-3xl">
                  <div className="flex items-center gap-3 mb-4">
                    ...
                    <span className="text-[12px] font-black uppercase tracking-widest text-white">
                      AI Sound Match Mastering
                    </span>
                  </div>
                  <p className="text-[9px] text-zinc-400 mb-4 uppercase font-bold tracking-tight">
                    Upload a reference track to match its spectral profile and LUFS density
                    automatically.
                  </p>
                  <button
                    onClick={() =>
                      handleSendMessage(
                        'Analyze reference track and apply AI Sound Match mastering.',
                      )
                    }
                ...
                <div className="p-6 bg-black/40 border border-white/5 rounded-3xl">
                  <div className="flex items-center justify-between mb-6">
                    <span className="text-[10px] font-black uppercase tracking-[0.2em] text-studio-accent">
                      Peak Mastering Dashboard
                    </span>
                    <div className="flex gap-2">
                      <span className="px-2 py-0.5 bg-studio-accent/20 rounded text-[7px] font-black text-studio-accent uppercase tracking-widest">
                        64-Bit DSP
                      </span>
                      <span className="px-2 py-0.5 bg-purple-500/20 rounded text-[7px] font-black text-purple-400 uppercase tracking-widest">
                        Neural Analog
                      </span>
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-3 mb-6">
                    {[
                      ...
                      <div
                        key={stat.name}
                        className="p-3 bg-zinc-950/80 rounded-2xl border border-white/5 flex flex-col items-center gap-1 shadow-lg"
                      >
                  ...
                  <div className="p-4 bg-gradient-to-br from-indigo-950/40 to-black border border-indigo-500/20 rounded-2xl">
                    <div className="flex items-center gap-3 mb-3">
                      ...
                      <span className="text-[9px] font-black uppercase tracking-widest text-indigo-200">
                        Neural 1176 Precision Modeling
                      </span>
                    </div>
          ...
          {activeTab === 'fx' && (
            ...
            >
              ...
              <div className="space-y-2">
                <span className="text-[8px] font-black uppercase text-zinc-500 tracking-[0.2em] mb-2 block">
                  VST Plugin Rack (Web-Compiled)
                </span>
                <div className="space-y-1">
                  {[
                    ...
                    <div
                      key={i}
                      className="p-3 bg-zinc-900/50 border border-white/5 rounded-xl flex items-center justify-between hover:border-studio-accent/50 transition-all cursor-pointer group"
                    >
                      <div className="flex items-center gap-3">
                        <div
                          className={`w-1.5 h-1.5 rounded-full ${vst.status === 'Active' ? 'bg-emerald-500 shadow-[0_0_5px_#10b981]' : 'bg-zinc-700'}`}
                        />
                        <div className="flex flex-col">
              ...
              <div className="grid grid-cols-2 gap-3">
                {[
                  ...
                  <button
                    key={fx.name}
                    className={`p-4 bg-zinc-900 border ${fx.color} rounded-2xl flex flex-col items-start gap-1 hover:border-studio-accent transition-all group`}
                  >
                    <span className="text-[10px] font-black uppercase group-hover:text-studio-accent">
                      {fx.name}
                    </span>
      ...
      <div className="p-5 border-t border-studio-border bg-black/80 backdrop-blur-xl flex items-center gap-6">
        ...
        <div className="flex-1 flex flex-col gap-2">
          <div className="h-1.5 bg-zinc-900 rounded-full overflow-hidden border border-white/5 relative">
            <div
              ref={progressBarRef}
              className="absolute inset-y-0 left-0 bg-studio-accent shadow-[0_0_10px_#3b82f6]"
            />
          </div>
