    ...
    <div className="fixed inset-0 z-[120] flex flex-col bg-[#020202]/98 backdrop-blur-3xl overflow-hidden animate-in fade-in slide-in-from-right-5 duration-500">
      ...
      <div className="h-16 border-b border-white/5 flex items-center justify-between px-8 bg-black/40 backdrop-blur-md">
        <div className="flex items-center gap-4">
          ...
          <div>
            <h2 className="text-sm font-black uppercase tracking-widest text-white">Collaborative Review</h2>
            <p className="text-[10px] text-zinc-500 font-bold uppercase tracking-tighter mt-0.5">Real-time Stakeholder Feedback Loop</p>
          </div>
        </div>
        ...
        <div className="flex items-center gap-6">
           <div className="flex -space-x-2">
              {(collaboration?.sessionUsers || []).map(u => (
                  <div key={u.id} className="w-8 h-8 rounded-full border-2 border-black bg-zinc-800 flex items-center justify-center overflow-hidden">
                      {u.avatar ? <img src={u.avatar} alt="" /> : <span className="text-[10px] font-black">{u.name[0]}</span>}
                  </div>
              ))}
           </div>
           <button 
      ...
      <div className="flex-1 flex overflow-hidden">
        ...
        <div className="flex-1 flex flex-col p-8 gap-6 bg-black/20">
             ...
             <div className="flex gap-4">
                 <button className="flex-1 py-4 bg-emerald-500/10 border border-emerald-500/20 rounded-2xl flex items-center justify-center gap-3 text-emerald-400 hover:bg-emerald-500/20 transition-all">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Approve Version</span>
                 </button>
                 <button className="flex-1 py-4 bg-rose-500/10 border border-rose-500/20 rounded-2xl flex items-center justify-center gap-3 text-rose-400 hover:bg-rose-500/20 transition-all">
                    ...
                    <span className="text-[10px] font-black uppercase tracking-widest">Request Revision</span>
                 </button>
             </div>
        </div>
        ...
        <div className="w-[450px] border-l border-white/5 bg-zinc-950/50 flex flex-col">
            <div className="p-6 border-b border-white/5">
                <h3 className="text-[11px] font-black uppercase tracking-widest text-zinc-400">Feedback Stream</h3>
            </div>
                        ...
                        <div key={comment.id} className="group space-y-3">
                            <div className="flex items-start justify-between">
                                <div className="flex items-center gap-2">
                                    <div className="w-6 h-6 rounded-full bg-indigo-600 flex items-center justify-center text-[8px] font-black text-white">{comment.authorName[0]}</div>
                                    <span className="text-[10px] font-black text-white uppercase">{comment.authorName}</span>
                                    <span className="text-[8px] font-mono text-zinc-600">at {comment.timecode.toFixed(2)}s</span>
                                </div>
                                <button className="opacity-0 group-hover:opacity-100 p-1 text-zinc-600 hover:text-white transition-all"><MoreVertical className="w-3 h-3" /></button>
                            </div>
            ...
            <div className="p-6 bg-black/40 border-t border-white/5">
                ...
                <div className="mt-3 flex items-center gap-2">
                    ...
                    <span className="text-[9px] font-mono text-zinc-600 uppercase">Marker at current playhead: <span className="text-white">{currentTime.toFixed(2)}s</span></span>
                </div>
            </div>
        </div>
      </div>
