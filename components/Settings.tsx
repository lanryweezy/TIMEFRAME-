...
import { Settings as SettingsIcon, Database, Zap, Palette, Save, User, Film, Download, Keyboard, X } from 'lucide-react';
      ...
      >
        ...
        <div className="flex-1 flex flex-col h-full bg-zinc-950">
            <div className="p-6 border-b border-zinc-800 flex items-center justify-between">
                <h3 className="text-lg font-bold text-white">{CATEGORIES.find(c => c.id === activeCategory)?.name}</h3>
                ...
                {activeCategory === 'general' && (
                    <div className="space-y-6">
                        <section className="space-y-4">
                            <h4 className="text-xs font-bold text-zinc-500 uppercase tracking-widest">Project Defaults</h4>
                         ...
                         <div className="space-y-4">
                            <label className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
                                <div>
                                    ...
                                    <div className="text-xs text-zinc-500">Accelerate rendering and AI inference.</div>
                                </div>
                                <input type="checkbox" className="accent-purple-500 w-5 h-5 cursor-pointer" defaultChecked />
                            </label>
                             <label className="flex items-center justify-between p-4 bg-zinc-900 border border-zinc-800 rounded-xl">
                                <div>
                                    <div className="text-white font-medium">Pre-cache Frames</div>
                                    <div className="text-xs text-zinc-500">Keep timeline frames in memory for smoother scrubbing.</div>
                                </div>
                                <input type="checkbox" className="accent-purple-500 w-5 h-5 cursor-pointer" defaultChecked />
                            </label>
                         </div>
            ...
            <div className="p-4 border-t border-zinc-800 flex justify-end gap-2 bg-zinc-900">
                <button onClick={onClose} className="px-6 py-2 bg-zinc-800 rounded-lg text-white hover:bg-zinc-700 transition-all">Cancel</button>
                <button className="px-6 py-2 bg-purple-600 rounded-lg text-white hover:bg-purple-500 transition-all flex items-center gap-2"><Save className="w-4 h-4" /> Save Changes</button>
            </div>
        </div>
      </motion.div>
    </div>
